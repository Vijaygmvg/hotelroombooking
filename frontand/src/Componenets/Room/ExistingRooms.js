    ...
    const handleDeleteRoom=async (roomId)=>{
      ...
      try{
           ...
           if(result===""){
            ...
            fetchRooms()
            alert("deleted room succesfullu ")
           }
           else{
        ...
        <tr key={room.id} className="hover:bg-pink-100 transition duration-300">
          ...
          <td className="py-3 px-6 border-b border-gray-200">
            <button className=" px-3 py-1 rounded mr-2 transition">
              ...
              <span className='bg-orange-300  min-w-6 justify-center ' ><FaEye/></span>
              <span className="bg-pink-600 min-w-6 justify-center"><Link to={`/edit/room/${room.id}`}><FaEdit/></Link></span>
              </div>
              
            </button>
            <button className="bg-white-500 hover:bg-red-600 text-white px-3 py-1 rounded transition" onClick={()=>handleDeleteRoom(room.id)}>
